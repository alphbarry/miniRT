...
int	parse_camera_orientation(t_camera *camera, char *orient_str)
{
    ...
    free_split(trid);
    // Normalizar el vector y validar rango [-1,1]
    if (!validate_numeric_range(camera->tridimensional.x, -1, 1, "Camera orientation x out of range") ||
        !validate_numeric_range(camera->tridimensional.y, -1, 1, "Camera orientation y out of range") ||
        !validate_numeric_range(camera->tridimensional.z, -1, 1, "Camera orientation z out of range"))
        return (0);
    return (1);
}
